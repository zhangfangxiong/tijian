...
abstract class Yaf_Controller
{
    ...
    protected function showMsg ($mMsg, $bRet, $redirectUrl = '')
    {
        ...
        $response = $this->getResponse();
        $response->appendBody(json_encode($aData, JSON_UNESCAPED_UNICDE | JSON_UNESCAPED_SLASHES));
        $this->autoRender(false);
     ...
     * @param unknown $aData            
     */
    protected function show404 ($sMsg = 'Not Found',$bRet=false, $redirectUrl = '')
    {
        if ($this->isAjax()) {
            return $this->showMsg($sMsg, $bRet, $redirectUrl);
        } else {
